  ...
  server.use(bodyParser.json()).post(
    ...
    sendWithTimeout(500, (req: CustomRequest<RequestGetAdminTokenPayload, any, any>, res) => {
      if (req.body.password === '111') {
        ...
        res.status(STATUS_UNPROCESSABLE_ENTITY).send(response);
      }
