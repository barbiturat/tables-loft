...
) => SessionDurationData = R.memoize((durationSeconds: number): SessionDurationData =>
...
const enhance = compose(
  ...
  withHandlers({
    onInputChange: ({ setHours, setMinutes }) => (event: ChangeEvent<HTMLInputElement>) => {
      const nameEquals = R.useWith(R.call, [R.identical, R.prop('name')]);
      const setTimePart = R.useWith(R.call, [R.identity, R.compose(Number, R.prop('value'))]);
  ...
  lifecycle<InnerProps, any>({
    componentWillReceiveProps(nextProps: PropsFromConnect) {
      ...
      R.unless(R.identical(this.props.durationSeconds), updateStates)(nextProps.durationSeconds);
    }
  })
);

const Component = enhance(({ hours, minutes, onInputChange, onInputKeyDown }: any) =>
