...
const requestLogin = (action$ => {
  return action$.ofType(REQUESTING_LOGIN).switchMap((action: FormSubmitAction) => {
      ...
      post(
        ...
        (
          ...
          if (ajaxData.status === STATUS_OK) {
            ...
            return Observable.of<BaseAction>(setSubmittedAction, redirectToIndexAction);
          } else {
            ...
            return Observable.of(setFormSubmitFailedAction, setFieldsValidityAction);
          }
        }
      )
    );
